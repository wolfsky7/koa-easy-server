...
const noteOptions = {
  ...
  getFunName: str => {
    ...
    const action = str.replace(/[\w\W]*?\(/, '').replace(/['",]/g, '').trim();
...
const analyRules = (file) => {
  fs.readFile(file, 'utf8', function (err, con) {
    ...
    const note = EasyNote(con, noteOptions);
    // 形如 @@{maxLength:1,minLength:2,required:true}
    const reg = /@@\{.*?\}/;
    for (const a in note) {
      const rules = [];
      if (a != "__title") {
        for (const f in note[a]) {
          ...
          if (!ruleDesc)
            continue;
          try {
            const _desc = /\{.*?:.*?\}/.exec(ruleDesc)[0].replace(/,/g, '","').replace(/\{/, '{\"').replace(/\}/, '\"}').replace(/:/g, '":"').replace('"[', '["').replace(']"', '"]');
...
const autoroute = (koa, dir) => {
  ...
  koa.use(async (ctx, next) => {
    const rk = `${ctx.request.path}-${ctx.request.method.toLowerCase()}`;
    const rules = _rules[rk];
    if (rules) {
